...
public class MyCustomSelectTextIndicator extends CommonPositionView {

    public static final String TAG = MyCustomSelectTextIndicator.class.getSimpleName();
    ...
    public MyCustomSelectTextIndicator(Context context) {
        super(context);
        initView(context);
    }

    public MyCustomSelectTextIndicator(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView(context);
    }

    public MyCustomSelectTextIndicator(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void initView(Context context){
        ...
        int color1 = rs.getColor(R.color.fanmenu_text_indicator_bg_color1);
        int color2 = rs.getColor(R.color.fanmenu_text_indicator_bg_color2);
        int color3 = rs.getColor(R.color.fanmenu_text_indicator_bg_color3);
        mWidth = rs.getDimensionPixelSize(R.dimen.fanmenu_select_text_bg_radius);
    ...
    public void setRotateView(int cur, float offset){
        ...
        switch (cur){
            case SelectCardState.CARD_STATE_FAVORATE:
                if(offset > 0.5){
                    tempCur = SelectCardState.CARD_STATE_RECENTLY;
                    offset -= 1;
                    break;
                }
                tempCur = cur;
            ...
            case SelectCardState.CARD_STATE_RECENTLY:
                if(offset < -0.5)
                {
                    tempCur = SelectCardState.CARD_STATE_FAVORATE;
                    offset +=1;
                    break;
                }
                tempCur = cur;
